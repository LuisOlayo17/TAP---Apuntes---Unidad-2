...
def generar_grafica_dispersion():
    # Simulando 20 lecturas de un sensor
    x = [i for i in range(20)]
    y = [random.randint(10, 50) for _ in range(20)]

    fig, ax = plt.subplots(figsize=(4, 3))

    ax.scatter(x, y, color="purple", alpha=0.8, edgecolors="white")
    ax.set_title("Ruido de Sensor", fontsize=10, weight="bold")
...
def generar_grafica_pastel():
    ...
    fig, ax = plt.subplots(figsize=(4, 3))
    ax.pie(valores, labels=categorias, autopct="%1.0f%%", startangle=90)
...
def main(page: ft.Page):
    ...
    header = ft.Text(
        "Dashboard de Visualización - Paso 3: Dispersión",
        size=24,
        weight="bold"
    )
    ...
    # --- ESPACIO 4: NUEVA GRÁFICA (PASTEL) ---
    ...
    # Agregamos los controles al tablero
